<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","textAlign":"left","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">房屋地址</label>
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#333","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					...
					<el-button class="btn18" v-if="isAuth('hetongxinxi','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('hetongxinxi','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('hetongxinxi','合同终止')" @click="hetongzhongzhiCrossAddOrUpdateHandler(scope.row,'cross','否','是','hetongzhuangtai','合同已终止','已终止,生效中'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								合同终止
							</el-button>
							<el-button class="btn8" v-if="isAuth('hetongxinxi','维修申请')" @click="weixiushenqingCrossAddOrUpdateHandler(scope.row,'cross','否','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								维修申请
							</el-button>
		...
		<hetongzhongzhi-cross-add-or-update v-if="hetongzhongzhiCrossAddOrUpdateFlag" :parent="this" ref="hetongzhongzhiCrossaddOrUpdate"></hetongzhongzhi-cross-add-or-update>
...
import hetongzhongzhiCrossAddOrUpdate from "../hetongzhongzhi/add-or-update";
import weixiushenqingCrossAddOrUpdate from "../weixiushenqing/add-or-update";
	export default {
		data() {
			return {
				...
				hetongzhongzhiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			hetongzhongzhiCrossAddOrUpdate,
		...
		methods: {
			...
			hetongzhongzhiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.hetongzhongzhiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.hetongzhongzhiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.hetongzhongzhiCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    init () {
		this.isPayOptions = "已支付,未支付".split(',')
          this.hetongzhuangtaiOptions = "已终止,生效中".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
