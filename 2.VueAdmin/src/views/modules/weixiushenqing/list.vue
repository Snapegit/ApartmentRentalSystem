<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","textAlign":"left","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="维修状态" prop="weixiuzhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">维修状态</label>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('weixiushenqing','立即处理')" @click="weixiuqingkuangCrossAddOrUpdateHandler(scope.row,'cross','是','','weixiuzhuangtai','已处理','已处理,未处理'.split(',')[0])" type="success">
		...
		<weixiuqingkuang-cross-add-or-update v-if="weixiuqingkuangCrossAddOrUpdateFlag" :parent="this" ref="weixiuqingkuangCrossaddOrUpdate"></weixiuqingkuang-cross-add-or-update>
...
import weixiuqingkuangCrossAddOrUpdate from "../weixiuqingkuang/add-or-update";
	export default {
		data() {
			return {
				...
				weixiuqingkuangCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			weixiuqingkuangCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			weixiuqingkuangCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.weixiuqingkuangCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.weixiuqingkuangCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.weixiuqingkuangCrossaddOrUpdate.init(row.id,type);
				});
			},
	...
	weixiushenqingstatusChange(e,row){
		if(row.status==0){
