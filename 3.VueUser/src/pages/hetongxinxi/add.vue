...
<div :style='{"width":"80%","padding":"20px","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="签订备注" prop="qiandingbeizhu">
            <el-input v-model="ruleForm.qiandingbeizhu" 
                placeholder="签订备注" clearable :disabled=" false  ||ro.qiandingbeizhu"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="合同状态" prop="hetongzhuangtai">
            <el-select v-model="ruleForm.hetongzhuangtai" placeholder="请选择合同状态" :disabled="true  ||ro.hetongzhuangtai" >
              <el-option
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='qiandingbeizhu'){
              this.ruleForm.qiandingbeizhu = obj[o];
              this.ro.qiandingbeizhu = true;
              continue;
            }
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`hetongxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('hetongxinxi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`hetongxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
