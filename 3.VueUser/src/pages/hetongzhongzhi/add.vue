    ...
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="登记编号" prop="dengjibianhao">
            <el-input v-model="ruleForm.dengjibianhao" 
                placeholder="登记编号" clearable :disabled=" false  ||ro.dengjibianhao"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="签订日期" prop="qiandingriqi">
            <el-input v-model="ruleForm.qiandingriqi" 
                placeholder="签订日期" clearable :disabled=" false  ||ro.qiandingriqi"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="合同附件" prop="hetongfujian">
            <file-upload
            tip="点击上传合同附件"
            action="file/upload"
            :limit="1"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="终止时间" prop="zhongzhishijian">
              <el-date-picker
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="终止原因" prop="zhongzhiyuanyin">
            <el-input v-model="ruleForm.zhongzhiyuanyin" 
                placeholder="终止原因" clearable :disabled=" false  ||ro.zhongzhiyuanyin"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    created() {
      ...
      this.ruleForm.zhongzhishijian = this.getCurDateTime()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='zhongzhiyuanyin'){
              this.ruleForm.zhongzhiyuanyin = obj[o];
              this.ro.zhongzhiyuanyin = true;
              continue;
            }
          }
        }else if(type=='edit'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`hetongzhongzhi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('hetongzhongzhi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`hetongzhongzhi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      hetongfujianUploadChange(fileUrls) {
          this.ruleForm.hetongfujian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
      },
    }
  };
</script>
